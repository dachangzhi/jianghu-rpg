...
SCENES['charm_farewell'] = () => {
    ...
    divider();
    showChoices([
        { text: '逐一告别（魅力≥13）', id: 'cf_b', req: [[() => req('charm', 13), '魅力≥13']], effects: () => {
                G.charm += 2;
                narrate(`你走到每个人面前——说了不同的话。
...
你摸了摸她的头：我会回来的。`);
            }, next: SCENES['lobby_free'] },
        { text: '不告别——悄悄走', id: 'cf_q', effects: () => {
                narrate(`你选择不告别——推门出去的那一刻——你回头看了一眼客栈。
...
你需要安慰她。`);
    divider();
    showChoices([
        { text: '温柔安慰（魅力≥9）', id: 'ccl_a', req: [[() => req('charm', 9), '魅力≥9']], effects: () => {
                G.charm += 1;
                narrate(`你蹲下来看着她：小莲——你知道苍龙镇为什么叫苍龙镇吗？因为很久以前有一条龙住在这里。龙保护着镇子。现在——龙不在了。但镇子还在。因为我们互相保护。你不用怕——大家都在。
...
你需要安抚他们。`);
    divider();
    showChoices([
        { text: '镇定人心（魅力≥14）', id: 'ctm_a', req: [[() => req('charm', 14), '魅力≥14']], effects: () => {
                G.charm += 2;
                narrate(`各位——天机卷上的名字是我们的家人、邻居、朋友。保护它——就是保护我们自己。苍龙镇从来没有被外人打倒过——今天也不会。
...
天机卷的争夺者同时到达了石室。你需要战斗、分析和说服同时进行。`);
    divider();
    showChoices([
        { text: '三合一应对（剑术≥16且心机≥14且魅力≥13）', id: 'cat_a', req: [[() => req('sword', 16), '剑术≥16'], [() => req('wits', 14), '心机≥14'], [() => req('charm', 13), '魅力≥13']], effects: () => {
                G.sword += 1;
                G.wits += 1;
                G.charm += 1;
                setFlag('mastered_all');
                narrate(`你用剑逼退了最危险的敌人——用心机看穿了谁是真正的威胁——用魅力说服了中立方加入你这边。
