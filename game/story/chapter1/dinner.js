...
SCENES['in_room'] = () => {
    ...
    const info = [];
    if (hasFlag('noticed_shen_skill') || hasFlag('confronted_shen'))
        info.push('沈孤雁：前武林高手，可能是前朝遗臣');
    if (hasFlag('knows_liu_identity'))
        info.push('柳如烟：锦衣卫暗探，她父亲在天机卷上');
    if (hasFlag('knows_zhao_past'))
        info.push('赵铁牛：前边军百夫长，厌倦杀戮归隐');
    if (hasFlag('bai_secret'))
        info.push('白云生：装醉自保，知道落雁峰的秘密');
    if (hasFlag('observed_all'))
        info.push('赵铁牛的铁锤藏有玄机，白云生在装醉');
    if (hasFlag('found_shen_routine'))
        info.push('沈孤雁每月十五会去某个地方');
    if (hasFlag('overheard_liu_comm'))
        info.push('柳如烟有传讯铜牌，三天期限');
    if (hasFlag('noticed_xiao_lian_medicine'))
        info.push('小莲懂草药，师父被朝廷暗探所杀');
    if (info.length > 0) {
...
SCENES['visit_pharmacy'] = () => {
    ...
    if (!G.relationships.hu_qingniang)
        G.relationships.hu_qingniang = 0;
...
SCENES['visit_riverside'] = () => {
    ...
    if (!G.relationships.old_sun)
        G.relationships.old_sun = 0;
    setFlag('met_old_sun');
    showChoices([
