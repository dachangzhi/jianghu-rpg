...
SCENES['talk_shen'] = () => {
    G.scene = 'talk_shen';
    if (!G.relationships.shen_guyan) {
        G.relationships.shen_guyan = 5;
    }
    else {
...
SCENES['talk_liu'] = () => {
    G.scene = 'talk_liu';
    if (!G.relationships.liu_ruyin)
        G.relationships.liu_ruyin = -3;
...
SCENES['talk_zhao'] = () => {
    G.scene = 'talk_zhao';
    if (!G.relationships.zhao_tieniu)
        G.relationships.zhao_tieniu = 8;
    else
...
SCENES['talk_bai'] = () => {
    G.scene = 'talk_bai';
    if (!G.relationships.bai_yunsheng)
        G.relationships.bai_yunsheng = 0;
...
SCENES['mountain_approach'] = () => {
    ...
    if (hasFlag('hei_alliance'))
        followers.push('黑无极');
    if (hasFlag('bai_following') || hasFlag('bai_key'))
        followers.push('白云生');
