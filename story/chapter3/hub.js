...
SCENES['chapter2_hub'] = () => {
    ...
    if (!hasFlag('got_hu_jade'))
        choices.push({ text: '去帮胡青娘解毒一个被下毒的人', id: 'c2h_hu', next: SCENES['hu_jade_quest'] });
    else
        choices.push({ text: '（已完成）胡青娘的玉牌已到手', id: 'c2h_hu_done', disabled: true });
    if (!hasFlag('got_bai_jade'))
        choices.push({ text: '帮白云生破解暗号', id: 'c2h_bai', next: SCENES['bai_jade_quest'] });
    else
        choices.push({ text: '（已完成）白云生的玉牌已到手', id: 'c2h_bai_done', disabled: true });
    if (!hasFlag('got_shen_jade'))
        choices.push({ text: '在天亮前找到沈孤雁的密室', id: 'c2h_shen', next: SCENES['shen_jade_quest'] });
    else
        choices.push({ text: '（已完成）沈孤雁的玉牌已到手', id: 'c2h_shen_done', disabled: true });
    // Other actions - only show if not done
    if (!hasFlag('got_zhao_sword'))
        choices.push({ text: '去铁匠铺帮赵铁牛修武器', id: 'c2h_zhao', effects: () => { changeRel('zhao_tieniu', 3); }, next: SCENES['zhao_weapon_help'] });
    if (!hasFlag('sun_confronted'))
        choices.push({ text: '去找老孙头谈谈', id: 'c2h_sun', next: SCENES['sun_confront'] });
    if (!hasFlag('know_final_attack'))
        choices.push({ text: '去茶馆找李婶买最新情报', id: 'c2h_li', next: SCENES['li_morning_intel'] });
    if (!hasFlag('patrolled_chapter2'))
        choices.push({ text: '在镇上巡逻确认安全', id: 'c2h_patrol', effects: () => { G.wits += 2; setFlag('patrolled_chapter2'); }, next: SCENES['chapter2_hub'] });
    if (!hasFlag('liu_private_talk_done'))
        choices.push({ text: '和柳如烟单独谈话', id: 'c2h_liu', next: SCENES['liu_private_talk'] });
    if (!hasFlag('tunnel_explored'))
        choices.push({ text: '检查地下通道入口', id: 'c2h_tunnel', next: SCENES['tunnel_explore'] });
    // Always available
    choices.push({ text: '准备出发上山', id: 'c2h_mountain', next: SCENES['mountain_prepare'] });
    if (!hasFlag('alliance_done'))
        choices.push({ text: '和所有人谈判结盟', id: 'c2h_alliance', next: SCENES['alliance_full'] });
    // Show jade count
    const kc = keyCount();
    if (kc > 0)
        narrate(`\n你目前拥有 ${kc}/3 块玉牌。`);
...
SCENES['bai_jade_quest'] = () => {
    ...
    showChoices([
        { text: '研究暗号（检定：才智>=7）', id: 'bjq_solve', effects: () => { if (G.wits >= 7) {
                setFlag('solved_cipher');
                G.wits += 3;
            }
            else {
                setFlag('failed_cipher');
            } }, next: SCENES['bai_jade_solve'] },
