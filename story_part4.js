...
SCENES['alliance_result'] = () => {
  ...
  const allies = [];
  if (getRel('shen_guyan') > 10 || hasFlag('shen_alliance')) allies.push('沈孤雁');
  if (getRel('liu_ruyin') > 5 || hasFlag('liu_alliance')) allies.push('柳如烟');
  if (getRel('zhao_tieniu') > 5) allies.push('赵铁牛');
  if (hasFlag('bai_key') || getRel('bai_yunsheng') > 5) allies.push('白云生');
