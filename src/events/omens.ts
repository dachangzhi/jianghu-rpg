...
SCENES['attack_begin'] = () => {
  ...
  showChoices([
    { text: '拔剑迎战（剑术检定1）', id: 'ab_f', effects: () => { if (G.sword >= 5) { setFlag('sword_check_1_pass'); } else { addHp(-15); setFlag('sword_check_1_fail'); } }, next: SCENES['attack_fight_1'] },
...
SCENES['attack_fight_1'] = () => {
  ...
  divider();
  showChoices([
    { text: '剑术检定2：凭听觉判断位置', id: 'af1_s', effects: () => { if (G.sword >= 7) { setFlag('sword_check_2_pass'); G.wits += 3; } else { addHp(-10); setFlag('sword_check_2_fail'); } }, next: SCENES['attack_fight_2'] },
...
SCENES['attack_shen_defense'] = () => {
  ...
  G.wits += 3;
  showChoices([
    { text: '剑术检定3：攻击屋顶杀手', id: 'asd_s', effects: () => { if (G.sword >= 9) { setFlag('sword_check_3_pass'); G.wits += 3; } else { addHp(-20); setFlag('sword_check_3_fail'); } }, next: SCENES['attack_retreat'] },
